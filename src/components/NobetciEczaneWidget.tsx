...
export default function NobetciEczaneWidget() {
  ...
  if (loading) {
    return (
      <div className="bg-white dark:bg-[#111] border border-gray-300 dark:border-[#262626] rounded-lg p-4">
  ...
  return (
    <div className="bg-white dark:bg-[#111] border border-gray-300 dark:border-[#262626] rounded-lg p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-xs text-gray-500 bg-gray-200 dark:bg-[#1a1a1a] px-2 py-1 rounded">
          Eskişehir
        </span>
      </div>
      ...
      <div className="space-y-3">
        {displayEczaneler.map((eczane) => (
          <div
            key={eczane.id}
            className="bg-gray-100 dark:bg-[#0a0a0a] border border-gray-300 dark:border-[#262626] rounded-lg p-3 hover:border-red-600/50 transition-colors"
          >
            <h4 className="font-semibold text-gray-900 dark:text-white text-sm mb-1">{eczane.ad}</h4>
            <p className="text-gray-600 dark:text-gray-400 text-xs mb-2 line-clamp-2">{eczane.adres}</p>
      ...
      {eczaneler.length > 3 && (
        <button
          ...
          className="w-full mt-3 text-center text-sm text-red-500 hover:text-red-400 transition-colors py-2 border-t border-gray-300 dark:border-[#262626]"
        >
          {expanded ? 'Daha az göster ↑' : `Tümünü göster (${eczaneler.length}) ↓`}
        </button>
      )}

      <div className="mt-3 pt-3 border-t border-gray-300 dark:border-[#262626] text-center">
        <a
          href="tel:182"
